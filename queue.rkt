...
; Implementați funcția rotate, conform axiomelor:
; rotate([], [y], Acc)        = y : Acc                    
; rotate((x:xs), (y:ys), Acc) = x : rotate(xs, ys, y : Acc)

(define (rotate left right Acc)
  (if (and (stream-empty? left) (null? (cdr right)))
      (stream-cons (car right) Acc)
      (stream-cons (stream-first left) (rotate (stream-rest left) (rest right) (stream-cons (first right) Acc)))))
...
(define (enqueue x q)
  (define queue2 (struct-copy queue q [right (cons x (queue-right q))] [size-r (+ 1 (queue-size-r q))]))
  (cond ((= (stream-length (queue-left queue2)) (- (length (queue-right queue2)) 1))
      ; aplic o rotatie
      (define queue3 (struct-copy queue queue2 [left (rotate (queue-left queue2) (queue-right queue2) empty-stream)]
                                  [size-l (+ (stream-length (queue-left queue2)) (length (queue-right queue2)))]
                                  [right '()] [size-r 0]))
      queue3)
        (else queue2)))
...
(define (dequeue q)
  (define queue2 (struct-copy queue q [left (stream-rest (queue-left q))] [size-l (- (queue-size-l q) 1)]))
   (cond ((= (stream-length (queue-left queue2)) (- (length (queue-right queue2)) 1))
      ; aplic o rotatie
      (define queue3 (struct-copy queue queue2 [left (rotate (queue-left queue2) (queue-right queue2) empty-stream)]
                                  [size-l (+ (stream-length (queue-left queue2)) (length (queue-right queue2)))]
